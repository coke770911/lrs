...
class Manage extends CI_Controller {
    ...
    public function __construct() {
        ...
        if($this->session->us_logid == null || $this->session->us_logid != "STAFF") {
            die("<script>alert('權限不足！');location.replace('/')</script>");
        }     
    ...
    public function exportList($id) {
        ...
        ->setCellValue( 'I1', '出生年月日')
        ->setCellValue( 'J1', '性別')
        ->setCellValue( 'K1', '行動電話')
        ->setCellValue( 'L1', '電子郵件')
        ->setCellValue( 'M1', '官網註冊')
        ->setCellValue( 'N1', '是否繳費')
        ->setCellValue( 'O1', '意見')
        ->setCellValue( 'P1', '成績')
        ->setCellValue( 'Q1', '是否合格');
        if(count($data) > 0) {
            $base_row = 2;
            foreach($data AS $key => $val) {
                ...
                ->setCellValue( 'I'.$row, $val['us_yy'].'/'.$val['us_mm'].'/'.$val['us_dd'])
                ->setCellValue( 'J'.$row, $val['ap_us_sex'] == 'F' ? '女' : '男')
                ->setCellValue( 'K'.$row, $val['ap_us_phone'])
                ->setCellValue( 'L'.$row, $val['ap_us_email'])
                ->setCellValue( 'M'.$row, $val['ap_is_regi'] == '0' ? '否' : '是')
                ->setCellValue( 'N'.$row, $this->tools->getCheckPayType($val['ap_is_pay']))
                ->setCellValue( 'O'.$row, $val['ap_us_memo'])
                ->setCellValue( 'P'.$row, $val['ap_score'])
                ->setCellValue( 'Q'.$row, $val['ap_scorePass'] == '0' ? '不合格' : '合格');
            }
        }

        header('Content-Type: application/vnd.ms-excel');
        header('Content-Disposition: attachment;filename="' . date('Ymd') . '證照報名資料.xls"');
    ...
    public function ThemeProcess() {
        ...
        $data = array(
            ...
            'rg_is_del' => $this->input->post("rg_is_del")
        );
